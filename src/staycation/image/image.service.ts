...
export class ImageService {
  ...
  async getImage(id: number) {
    ...
    if (!image) throw new NotFoundException(ResponseMessage.IMAGE_NOT_FOUND)

    return new Result(ResponseMessage.SUCCESS, image)
  }

  async createImage(imageUrl: string) {
    const newImage = this.imageRepository.create({ imageUrl })
    
    return new Result(ResponseMessage.SUCCESS, await this.imageRepository.save(newImage))
  }
